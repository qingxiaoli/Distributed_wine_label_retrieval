...
def process_img(im, name):
    ...
    mask = cv2.imread('/home/wangya/wine/distributed/data/files_1/test_mask/%s'%name.replace('.jpg','.png'), 0)
...
def do(x, n):
    train_path = '/home/wangya/wine/distributed/data/files_1/test'
    train_pics = os.listdir(train_path)
    train_save_path = '/home/wangya/wine/distributed/data/files_1/pysift_test'
    for im in train_pics[x:x+n]:
        name = im.replace('.jpg', '.pkl')
        if os.path.exists('/home/wangya/wine/distributed/data/files_1/pysift_test/'+name) or not im.endswith('.jpg'):
            continue
